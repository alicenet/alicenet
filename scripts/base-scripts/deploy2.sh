...
npx hardhat --network "$NETWORK" --show-stack-traces deploy-contracts --config-file ../scripts/base-files/deploymentConfig2.json --skip-checks 2>&1 | tee ../scripts/generated/deployconfig2.txt

# -f  will check for the file existence but -s will check for file existence along with file size greater than 0 (zero).
if [[ -s ../scripts/generated/deployconfig2.txt ]]; then
    addr=$(go run ../cmd/testutils/extractor/main.go -p ../scripts/generated/deployconfig2.txt)
else
    echo "deployconfig file doesn't exist in scripts/generated/deployconfig2.txt path. Exiting..."
    exit 1
fi
