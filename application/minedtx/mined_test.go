...
func makeTxInitial(t *testing.T, ownerSigner objs.Signer, numTxIns uint32) (objs.Vout, *objs.Tx) {
	t.Helper()
	if numTxIns == 0 {
...
func TestMined(t *testing.T) {
	...
	consumedUTXOs, tx := makeTxInitial(t, ownerSigner, 2)
	consumedUTXOs2, tx2 := makeTxInitial(t, ownerSigner, 3)
	...
	err = db.Update(func(txn *badger.Txn) error {
		...
		getTx1, missing, err := hndlr.Get(txn, [][]byte{txHash})
		if err != nil {
			t.Fatal(err)
		}
		if len(missing) > 0 {
		...
		getTx2, missing, err := hndlr.Get(txn, [][]byte{tx2Hash})
		if err != nil {
			t.Fatal(err)
		}
		if len(missing) > 0 {
		...
		if len(missing) > 0 {
			t.Fatal("Should not be missing because tx was not deleted")
		}
		atHeight, err = hndlr.GetHeightForTx(txn, getTxHash1)
		if err != nil {
			t.Fatal(err)
		...
		height := uint32(1)
		idx := uint32(0)
		retTxhash, err := hndlr.heightIdxIndex.GetTxHashFromHeightIdx(txn, height, idx)
		if err != nil {
			t.Fatal(err)
		}
		if !bytes.Equal(retTxhash, txHash) {
			t.Fatal("incorrect hash returned")
		}
		idx = uint32(1)
...
func TestMinedDelete(t *testing.T) {
	...
	ownerSigner := testingOwner(t)
	numTxIns := uint32(3)
	consumedUTXOs, tx := makeTxInitial(t, ownerSigner, numTxIns)
...
func TestMinedGetOneInternal(t *testing.T) {
	...
	err = db.Update(func(txn *badger.Txn) error {
		...
		err = hndlr.addOneInternal(txn, tx, txHash, height, idx)
		if err != nil {
			t.Fatal(err)
		}
...
func TestMinedAddOneInternal(t *testing.T) {
	...
	err = db.Update(func(txn *badger.Txn) error {
		idx := uint32(0)
		err := hndlr.addOneInternal(txn, txBad, txHashBad, height, idx)
		if err == nil {
			t.Fatal("Should have raised error")
		}
		return nil
