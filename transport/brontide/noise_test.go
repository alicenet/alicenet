...
func makeListener() (*Listener, *NetAddress, error) {
	// First, generate the long-term private keys for the brontide listener.
	localPriv, err := secp256k1.NewPrivateKey(secp256k1.S256())
	if err != nil {
		return nil, nil, err
	}
	...
	listener, err := NewListener(localPriv, addr, testPortListener, testProtoVer, testChainID, 50, 1, 50)
	if err != nil {
		return nil, nil, err
	}
...
func establishTestConnection(t *testing.T) (net.Conn, net.Conn, func(), error) {
	...
	// successful.
	remoteConnChan := make(chan maybeNetConn)
	go func() {
	...
	localConnChan := make(chan maybeNetConn)
	go func() {
...
func TestConnectionCorrectness(t *testing.T) {
	...
	localConn, remoteConn, cleanUp, err := establishTestConnection(t)
	if err != nil {
		t.Fatalf("unable to establish test connection: %v", err)
	}
	if localConn == nil {
		t.Fatal("Got a nil conn")
	}
	if remoteConn == nil {
		t.Fatal("Got a nil conn")
	}
...
func TestBolt0008TestVectors(t *testing.T) {
	...
	initiatorPriv, _ := secp256k1.PrivKeyFromBytes(secp256k1.S256(),
		initiatorKeyBytes)
	...
	initiatorEphemeral := EphemeralGenerator(func() (*secp256k1.PrivateKey, error) {
		...
		priv, _ := secp256k1.PrivKeyFromBytes(secp256k1.S256(), eBytes)
		return priv, nil
	})
