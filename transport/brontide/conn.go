...
func (c *Conn) wrapClose(newClose func() error) {
	c.closeFn = newClose
}
...
func Dial(localPriv *secp256k1.PrivateKey, protocol types.Protocol, protoVersion types.ProtoVersion, chainID types.ChainIdentifier, port int, netAddr *NetAddress, dialer func(string, string) (net.Conn, error)) (*Conn, error) {
	...
	if err := selfInitiatedChainIdentifierHandshake(b, chainID); err != nil {
		b.conn.Close()
		return nil, err
	}
	...
	remoteP2PPort, err := selfInitiatedPortHandshake(b, port)
	if err != nil {
		b.conn.Close()
		return nil, err
	}
	...
	remoteVersion, err := selfInitiatedVersionHandshake(b, protoVersion)
	if err != nil {
		b.conn.Close()
		return nil, err
	}

	if err := writeUint32(b, uint32(protocol)); err != nil {
		b.conn.Close()
		return nil, err
	}
