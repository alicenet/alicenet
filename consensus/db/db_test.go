...
func TestStagedBHKeyIter(t *testing.T) {
	groupSigner := &crypto.BNGroupSigner{}
	groupSigner.SetPrivk(crypto.Hasher([]byte("secret")))
	...
	err := badgerD.Update(func(txn *badger.Txn) error {
		sbhki := db.GetStagedBlockHeaderKeyIter(txn)
		defer sbhki.Close()
		_, _, isDone, err := sbhki.Next()
		if err != nil {
			t.Fatal(err)
		}
		if !isDone {
			t.Fatal("Should be done because nothing to iterate")
		}
		return nil
	})
	if err != nil {
	...
	err = badgerD.Update(func(txn *badger.Txn) error {
		...
		for height := uint32(1); height < 1025; height++ {
			bh := &objs.BlockHeader{
				SigGroup: sig,
				BClaims: &objs.BClaims{
					ChainID:    p.ChainID,
			...
			*/
			...
			//err = db.SetSnapshotBlockHeader(txn, bh)
			err = db.SetStagedBlockHeader(txn, height, bhBytes)
			if err != nil {
				t.Fatal(err)
			}
		}
