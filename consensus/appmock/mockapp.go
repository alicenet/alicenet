...
func (m *MockApplication) PendingTxAdd(txn *badger.Txn, chainID, height uint32, txs []interfaces.Transaction) error {
	if chainID == 7777 {
...
func (m *MockApplication) IsValid(txn *badger.Txn, chainID, height uint32, stateHash []byte, _ []interfaces.Transaction) (bool, error) {
	if chainID == 7777 {
...
func (m *MockApplication) StoreSnapShotNode(txn *badger.Txn, batch, root []byte, layer int) ([][]byte, int, []trie.LeafNode, error) {
	if m.shouldFail {
...
func (m *MockApplication) StoreSnapShotStateData(txn *badger.Txn, key, value, data []byte) error {
	if m.shouldFail {
