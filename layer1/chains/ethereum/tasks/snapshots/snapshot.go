...
func (t *SnapshotTask) Prepare(ctx context.Context) *tasks.TaskErr {
	...
	if err != nil {
		return tasks.NewTaskErr(
			fmt.Sprintf("unable to marshal block header for snapshot: %v", err),
			false,
		)
	}
	...
	height, err := t.GetContractsHandler().
		EthereumContracts().
		Snapshots().
		GetCommittedHeightFromLatestSnapshot(opts)
	if err != nil {
		return tasks.NewTaskErr(
			fmt.Sprintf("failed to determine committed height: %v", err),
			true,
		)
	}

	desperationFactor, err := t.GetContractsHandler().
		EthereumContracts().
		Snapshots().
		GetSnapshotDesperationFactor(opts)
	if err != nil {
		return tasks.NewTaskErr(
			fmt.Sprintf("failed to determine desperation factor: %v", err),
			true,
		)
	}

	desperationDelay, err := t.GetContractsHandler().
		EthereumContracts().
		Snapshots().
		GetSnapshotDesperationDelay(opts)
	if err != nil {
		return tasks.NewTaskErr(
			fmt.Sprintf("failed to determine desperation delay: %v", err),
			true,
		)
	}
...
func (t *SnapshotTask) Execute(ctx context.Context) (*types.Transaction, *tasks.TaskErr) {
	...
	if err != nil {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf("error getting eth height for leader election: %v", err),
			true,
		)
	}
	if !isLeading {
	...
	txn, err := t.GetContractsHandler().
		EthereumContracts().
		Snapshots().
		Snapshot(txnOpts, snapshotState.RawSigGroup, snapshotState.RawBClaims)
	if err != nil {
...
func (t *SnapshotTask) ShouldExecute(ctx context.Context) (bool, *tasks.TaskErr) {
	...
	height, err := t.GetContractsHandler().
		EthereumContracts().
		Snapshots().
		GetAliceNetHeightFromLatestSnapshot(opts)
	if err != nil {
	...
	if snapshotState.BlockHeader.BClaims.Height != 0 &&
		snapshotState.BlockHeader.BClaims.Height <= uint32(height.Uint64()) {
		logger.Debugf(
