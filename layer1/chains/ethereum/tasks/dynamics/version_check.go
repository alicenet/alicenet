...
func (t *CanonicalVersionCheckTask) Execute(
	ctx context.Context,
) (*types.Transaction, *tasks.TaskErr) {
	logger := t.GetLogger().WithField("method", "Execute()")
	logger.Debug("initiate execution")

	newMajorIsGreater, newMinorIsGreater, newPatchIsGreater, localVersion, err := utils.CompareCanonicalVersion(
		t.Version,
	)
	if err != nil {
	...
	logger = logger.WithField(
		"currentVersion",
		fmt.Sprintf("%d.%d.%d", localVersion.Major, localVersion.Minor, localVersion.Patch),
	)
