...
func (t *KeyShareSubmissionTask) Prepare(ctx context.Context) *tasks.TaskErr {
	...
	if isKeyShareNil ||
		(dkgState.Participants[defaultAddr].KeyShareG1s[0].Cmp(big.NewInt(0)) == 0 &&
...
func (t *KeyShareSubmissionTask) Execute(
	ctx context.Context,
...
func (t *KeyShareSubmissionTask) ShouldExecute(ctx context.Context) (bool, *tasks.TaskErr) {
	...
	status, err := state.CheckKeyShare(
		ctx,
		t.GetContractsHandler().EthereumContracts().Ethdkg(),
		logger,
		callOpts,
		defaultAccount.Address,
		dkgState.Participants[defaultAccount.Address].KeyShareG1s,
	)
	if err != nil {
