...
import (
	...
	"github.com/alicenet/alicenet/layer1/chains/ethereum/tasks/dkg/utils"
...
func (t *DisputeShareDistributionTask) Execute(
	ctx context.Context,
) (*types.Transaction, *tasks.TaskErr) {
	...
	if dkgState.Phase != state.DisputeShareDistribution &&
		dkgState.Phase != state.ShareDistribution {
		return nil, tasks.NewTaskErr(
			"it's not DisputeShareDistribution or ShareDistribution phase",
			false,
		)
	}
	...
	if participantState == nil {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf("couldn't find %v in the dkgState ParticipantList", t.Address.Hex()),
			false,
		)
	}
	...
	if err != nil {
		...
		return nil, tasks.NewTaskErr(
			fmt.Sprintf(
				"VerifyDistributedShares broke: %v Participant Address: %v",
				err.Error(),
				participantState.Address.Hex(),
			),
			false,
		)
	}
	...
	if err != nil {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf("failed generating disputePublicKeyG1: %v", err),
			false,
		)
	}
	sharedKeyG1 := cloudflare.GenerateSharedSecretG1(
		dkgState.TransportPrivateKey,
		disputePublicKeyG1,
	)
	sharedKey, err := bn256.G1ToBigIntArray(sharedKeyG1)
	if err != nil {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf("failed generating sharedKeyG1: %v", err),
			false,
		)
	}
	...
	if err != nil {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf("failed generating sharedKeyProof: %v", err),
			false,
		)
	}
	...
	// Accuse participant
	txn, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		AccuseParticipantDistributedBadShares(txnOpts, dishonestAddress, encryptedShares, commitments, sharedKey, sharedKeyProof)
	if err != nil {
...
func (t *DisputeShareDistributionTask) ShouldExecute(
	ctx context.Context,
) (bool, *tasks.TaskErr) {
	...
	isValidator, err := utils.IsValidator(t.GetDB(), logger, t.Address)
	if err != nil {
		return false, tasks.NewTaskErr(
			fmt.Sprintf(tasks.FailedGettingIsValidator, err),
			false,
		)
	}
	logger.WithFields(logrus.Fields{"eth.badParticipant": t.Address.Hex()}).
		Debug("participant was not accused yet")
