...
func (t *GPKjSubmissionTask) Execute(ctx context.Context) (*types.Transaction, *tasks.TaskErr) {
	...
	txn, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		SubmitGPKJ(txnOpts, dkgState.Participants[dkgState.Account.Address].GPKj)
	if err != nil {
...
func (t *GPKjSubmissionTask) ShouldExecute(ctx context.Context) (bool, *tasks.TaskErr) {
	...
	participantState, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		GetParticipantInternalState(callOpts, defaultAddr.Address)
	if err != nil {
		return false, tasks.NewTaskErr(
			fmt.Sprintf("failed getting participants state: %v", err),
			true,
		)
	}
