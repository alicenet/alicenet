...
import (
	...
	dkgTestUtils "github.com/alicenet/alicenet/layer1/chains/ethereum/tasks/tests/utils"
...
func TestMath_VerifyDistributedSharesGood1(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		...
		for partIdx := 0; partIdx < n; partIdx++ {
			...
			if err != nil {
				t.Fatalf(
					"Error raised in VerifyDistributedShares: s_i->j; i: %v; j: %v\nerr:= %v\n",
					participant.Index,
					dkgState.Index,
					err,
				)
			...
			if !present {
				t.Fatalf(
					"Invalid share in VerifyDistributedShares: s_i->j; i: %v; j: %v\nnot present\n",
					participant.Index,
					dkgState.Index,
				)
			}
...
func TestMath_VerifyDistributedSharesGood3(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		...
		if valid {
			t.Fatalf(
				"Valid share in VerifyDistributedShares: s_i->j; i: %v; j: %v\n",
				badParticipant.Index,
				dkgState.Index,
			)
		}
...
func TestMath_CategorizeGroupSignersBad(t *testing.T) {
	...
	_, _, _, err = state.CategorizeGroupSigners(
		publishedPublicKeysBad,
		participants,
		commitmentArray,
	)
	if err == nil {
	...
	for k := 0; k < n; k++ {
		publishedPublicKeysBad2 = append(
			publishedPublicKeysBad2,
			[4]*big.Int{big.NewInt(1), big.NewInt(1), big.NewInt(1), big.NewInt(1)},
		)
	}
	_, _, _, err = state.CategorizeGroupSigners(
		publishedPublicKeysBad2,
...
func setupGroupSigners(
	...
	for _, participant := range participants {
		...
		participantEncryptedShares, participantPrivateCoefficients, commitments, err := state.GenerateShares(
			privateKey,
			participants,
		)
		assert.Nil(t, err)
	...
	for idx, participant := range participants {
		...
		_, groupPublicKey, err := state.GenerateGroupKeys(
			privateKey,
			privateCoefficients[idx],
			encryptedShares,
			participant.Index,
			participants,
		)
