...
func (state *DkgState) OnRegistrationOpened(
...
func (state *DkgState) OnAddressRegistered(
...
func (state *DkgState) OnSharesDistributed(
	logger *logrus.Entry,
	account common.Address,
	...
	if err != nil {
		logger.Errorf(
			"ProcessShareDistribution: error calculating distributed shares hash: %v",
			err,
		)
		return err
	}
...
func (state *DkgState) OnKeyShareSubmitted(
	account common.Address,
	keyShareG1, keyShareG1CorrectnessProof [2]*big.Int,
...
func CategorizeGroupSigners(
	...
	commitments [][][2]*big.Int,
) (ParticipantList, ParticipantList, ParticipantList, error) {
	...
	if n != len(publishedPublicKeys) || n != len(commitments) {
		return ParticipantList{}, ParticipantList{}, ParticipantList{}, fmt.Errorf(
			"mismatched public keys (%v), participants (%v), commitments (%v)",
			len(publishedPublicKeys),
			n,
			len(commitments),
		)
	}
	...
	for idx := 0; idx < n; idx++ {
		...
		if err != nil {
			return ParticipantList{}, ParticipantList{}, ParticipantList{}, fmt.Errorf(
				"error converting BigIntArray to G2: %v",
				err,
			)
		}
		...
		for polyDegreeIdx := 0; polyDegreeIdx <= threshold; polyDegreeIdx++ {
			...
			for participantIdx := 0; participantIdx < n; participantIdx++ {
				...
				if err != nil {
					return ParticipantList{}, ParticipantList{}, ParticipantList{}, fmt.Errorf(
						"error converting BigIntArray to G1: %v",
						err,
					)
				}
				tmp1.Add(tmp1, tmp2)
		...
		validPair := cloudflare.PairingCheck(
			[]*cloudflare.G1{gpkjStar, g1Base},
			[]*cloudflare.G2{h2Neg, gpkj},
		)
		if validPair {
