...
func (t *RegisterTask) Execute(ctx context.Context) (*types.Transaction, *tasks.TaskErr) {
	...
	// Register
	logger.Infof(
		"Registering  publicKey (%v) with ETHDKG",
		utils.FormatPublicKey(dkgState.TransportPublicKey),
	)
	logger.Debugf(
		"registering on block %v with public key: %v",
		block,
		utils.FormatPublicKey(dkgState.TransportPublicKey),
	)
	txn, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		Register(txnOpts, dkgState.TransportPublicKey)
	if err != nil {
...
func (t *RegisterTask) ShouldExecute(ctx context.Context) (bool, *tasks.TaskErr) {
	...
	status, err := state.CheckRegistration(
		t.GetContractsHandler().EthereumContracts().Ethdkg(),
		logger,
		callOpts,
