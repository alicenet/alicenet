...
func (t *DisputeMissingRegistrationTask) Execute(
	ctx context.Context,
	...
	txn, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		AccuseParticipantNotRegistered(txnOpts, accusableParticipants)
	if err != nil {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf("error accusing missing registration: %v", err),
			true,
		)
	}
...
func (t *DisputeMissingRegistrationTask) ShouldExecute(
	ctx context.Context,
) (bool, *tasks.TaskErr) {
...
func (t *DisputeMissingRegistrationTask) getAccusableParticipants(
