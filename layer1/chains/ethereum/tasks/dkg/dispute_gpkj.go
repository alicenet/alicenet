...
func (t *DisputeGPKjTask) Execute(ctx context.Context) (*types.Transaction, *tasks.TaskErr) {
	...
	if dkgState.Phase != state.DisputeGPKJSubmission && dkgState.Phase != state.GPKJSubmission {
		return nil, tasks.NewTaskErr(
			"it's not DisputeGPKJSubmission or GPKJSubmission phase",
			false,
		)
	}
	...
	honest, dishonest, missing, err := state.CategorizeGroupSigners(
		groupPublicKeys,
		participantList,
		groupCommitments,
	)
	if err != nil {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf("failed to determine honest vs dishonest validators: %v", err),
			false,
		)
	}
	...
	if len(honest)+len(missing)+len(dishonest) != len(participantList) {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf(
				"missing information when computing honest, dishonest, missing validators: %v",
				err,
			),
			false,
		)
	...
	for _, dishonestParticipant := range dishonest {
		if bytes.Equal(dishonestParticipant.Address.Bytes(), t.Address.Bytes()) {
			return t.accuseDishonestValidator(
				ctx,
				logger,
				dkgState,
				participantList,
...
func (t *DisputeGPKjTask) accuseDishonestValidator(
	ctx context.Context,
	logger *logrus.Entry,
	...
	txn, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		AccuseParticipantSubmittedBadGPKJ(txnOpts, validatorAddresses, groupEncryptedSharesHash, groupCommitments, t.Address)
	if err != nil {
