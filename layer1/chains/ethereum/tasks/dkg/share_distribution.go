	...
	if dkgState.SecretValue == nil {
		...
		if err != nil {
			return tasks.NewTaskErr(
				fmt.Sprintf("Failed to generate shares: %v %#v", err, participants),
				true,
			)
		}
...
func (t *ShareDistributionTask) Execute(
	ctx context.Context,
...
func (t *ShareDistributionTask) ShouldExecute(ctx context.Context) (bool, *tasks.TaskErr) {
	...
	participantState, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		GetParticipantInternalState(callOpts, dkgState.Account.Address)
	if err != nil {
		return false, tasks.NewTaskErr(
			fmt.Sprintf("unable to GetParticipantInternalState(): %v", err),
			true,
		)
	}
