...
func (t *MPKSubmissionTask) Prepare(ctx context.Context) *tasks.TaskErr {
	...
	if isMasterPublicKeyEmpty(dkgState.MasterPublicKey) {
		...
		if err != nil {
			return tasks.NewTaskErr(
				fmt.Sprintf("Failed to generate master public key:%v", err),
				false,
			)
		}
...
func (t *MPKSubmissionTask) Execute(ctx context.Context) (*types.Transaction, *tasks.TaskErr) {
	...
	txn, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		SubmitMasterPublicKey(txnOpts, dkgState.MasterPublicKey)
	if err != nil {
		return nil, tasks.NewTaskErr(
			fmt.Sprintf("submitting master public key failed: %v", err),
			true,
		)
	}
...
func (t *MPKSubmissionTask) ShouldExecute(ctx context.Context) (bool, *tasks.TaskErr) {
	...
	mpkHash, err := t.GetContractsHandler().
		EthereumContracts().
		Ethdkg().
		GetMasterPublicKeyHash(callOpts)
	if err != nil {
		return false, tasks.NewTaskErr(
			fmt.Sprintf("failed to retrieve mpk from smart contracts: %v", err),
			true,
		)
	}
	...
	if err != nil {
		return false, tasks.NewTaskErr(
			fmt.Sprintf("failed to serialize internal mpk: %v", err),
			false,
		)
	}
