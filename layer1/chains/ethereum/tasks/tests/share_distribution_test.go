...
func TestShareDistribution_Group_1_Good(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		...
		err := shareDistributionTask.Initialize(
			...
			"task-id",
			shareDistributionTask.Start,
			shareDistributionTask.End,
			false,
			nil,
			nil,
		)
...
func TestShareDistribution_Group_2_Bad6(t *testing.T) {
	...
	assert.Nil(t, err)
	task := dkg.NewShareDistributionTask(
		dkgState.PhaseStart,
		dkgState.PhaseStart+dkgState.PhaseLength,
	)
	err = task.Initialize(
		dkgDb,
		fixture.Logger,
