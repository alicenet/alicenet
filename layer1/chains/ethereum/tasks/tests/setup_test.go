...
import (
	...
	testutils "github.com/alicenet/alicenet/layer1/chains/ethereum/tasks/tests/utils"
...
func SetETHDKGPhaseLength(
	...
	txn, err := fixture.Contracts.EthereumContracts().
		ContractFactory().
		CallAny(callOpts, fixture.Contracts.EthereumContracts().EthdkgAddress(), big.NewInt(0), input)
	if err != nil {
		return nil, nil, err
	}
	if txn == nil {
		return nil, nil, errors.New(
			"non existent transaction ContractFactory.CallAny(ethdkg, setPhaseLength(...))",
		)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf(
			"error getting receipt for ContractFactory.CallAny(ethdkg, setPhaseLength(...)) err: %v",
			err,
		)
	}
	return txn, rcpt, nil
...
func InitializeETHDKG(
	...
	txn, err := fixture.Contracts.EthereumContracts().
		ContractFactory().
		CallAny(callOpts, fixture.Contracts.EthereumContracts().ValidatorPoolAddress(), big.NewInt(0), input)
	if err != nil {
		return nil, nil, err
	}
	if txn == nil {
		return nil, nil, errors.New(
			"non existent transaction ContractFactory.CallAny(validatorPool, initializeETHDKG())",
		)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf(
			"error getting receipt for ContractFactory.CallAny(validatorPool, initializeETHDKG()) err: %v",
			err,
		)
	}
...
func StartFromRegistrationOpenPhase(
	t *testing.T,
	fixture *tests.ClientFixture,
	...
	valCount, err := fixture.Contracts.EthereumContracts().
		ValidatorPool().
		GetValidatorsCount(callOpts)
	assert.Nil(t, err)
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < n; j++ {
			...
			assert.Nil(t, err)
			otherDkgState.OnAddressRegistered(
				dkgState.Account.Address,
				i+1,
				dkgState.Nonce,
				dkgState.TransportPublicKey,
			)
	...
	if unregisteredValidators == 0 {
		...
		for idx := 0; idx < n; idx++ {
			...
			assert.Nil(t, err)
			shareDistributionTask, disputeMissingShareDistributionTask, disputeShareDistTask := events.UpdateStateOnRegistrationComplete(
				dkgState,
				height,
			)
...
func StartFromShareDistributionPhase(
	t *testing.T,
	fixture *tests.ClientFixture,
	undistributedSharesIdx []int,
	badSharesIdx []int,
	phaseLength uint16,
) *EthDkgTestSuite {
	...
	for idx := 0; idx < n; idx++ {
		...
		err = shareDistTask.Initialize(
			suite.DKGStatesDbs[idx],
			logger,
			suite.Eth,
			fixture.Contracts,
			"ShareDistributionTask",
			fmt.Sprintf("%v", idx),
			shareDistTask.Start,
			shareDistTask.End,
			false,
	...
	if len(undistributedSharesIdx) == 0 {
		...
		for i := 0; i < n; i++ {
			...
			assert.Nil(t, err)
			disputeShareDistributionTask, keyshareSubmissionTask, disputeMissingKeySharesTask := events.UpdateStateOnShareDistributionComplete(
				dkgState,
				height,
			)
...
func StartFromKeyShareSubmissionPhase(
	t *testing.T,
	fixture *tests.ClientFixture,
	...
	for idx := 0; idx < n; idx++ {
		...
		err := keyshareSubmissionTask.Initialize(
			suite.DKGStatesDbs[idx],
			logger,
			suite.Eth,
			fixture.Contracts,
			"KeyShareSubmissionTask",
			fmt.Sprintf("%v", idx),
			keyshareSubmissionTask.Start,
			keyshareSubmissionTask.End,
			false,
			nil,
			nil,
		)
...
func StartFromMPKSubmissionPhase(
	t *testing.T,
	fixture *tests.ClientFixture,
	phaseLength uint16,
) *EthDkgTestSuite {
	...
	for idx := 0; idx < numValidators; idx++ {
		dkgState, err := state.GetDkgState(suite.DKGStatesDbs[idx])
		assert.Nil(t, err)
		gpkjSubmissionTask, disputeMissingGPKjTask, disputeGPKjTask := events.UpdateStateOnMPKSet(
			dkgState,
			height,
...
func StartFromGPKjPhase(
	t *testing.T,
	fixture *tests.ClientFixture,
	undistributedGPKjIdx []int,
	badGPKjIdx []int,
	phaseLength uint16,
) *EthDkgTestSuite {
	...
	if len(undistributedGPKjIdx) == 0 {
		...
		for i := 0; i < n; i++ {
			dkgState, err := state.GetDkgState(suite.DKGStatesDbs[i])
			assert.Nil(t, err)
			disputeGPKjTask, completionTask := events.UpdateStateOnGPKJSubmissionComplete(
				dkgState,
				height,
			)
...
func CompleteEthDkgCeremony(
	...
	for idx := 0; idx < numValidators; idx++ {
		for j := 0; j < numValidators; j++ {
			...
			err := disputeGPKjTask.Initialize(
				suite.DKGStatesDbs[idx],
				fixture.Logger,
				suite.Eth,
				fixture.Contracts,
...
func RegisterPotentialValidatorOnMonitor(
	t *testing.T,
...
func CheckBadValidators(
	...
	for _, badId := range badValidators {
		...
		isValidator, err := contracts.EthereumContracts().
			ValidatorPool().
			IsValidator(callOpts, dkgState.Account.Address)
		assert.Nil(t, err)
