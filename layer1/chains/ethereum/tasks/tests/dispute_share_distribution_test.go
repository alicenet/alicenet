...
func TestDisputeShareDistributionTask_Group_1_OneValidatorSubmittingInvalidCredentials(
	t *testing.T,
) {
	n := 5
	badId := 4
	...
	for idx := 0; idx < n; idx++ {
		for j := 0; j < n; j++ {
			...
			if suite.BadAddresses[task.Address] {
				isValidator, err := utils.IsValidator(
					suite.DKGStatesDbs[idx],
					task.GetLogger(),
					task.Address,
				)
	...
	badParticipants, err := fixture.Contracts.EthereumContracts().
		Ethdkg().
		GetBadParticipants(callOptions)
	assert.Nil(t, err)
...
func TestDisputeShareDistributionTask_Group_1_TwoValidatorSubmittingInvalidCredentials(
	t *testing.T,
) {
	n := 5
	b := []int{2, 4}
...
func TestDisputeShareDistributionTask_Group_1_AllValidatorSubmittingInvalidCredentials(
	t *testing.T,
) {
	n := 5
	b := []int{0, 1, 2, 3, 4}
