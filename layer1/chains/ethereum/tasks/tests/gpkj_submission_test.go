...
func TestGPKjSubmission_Group_1_GoodAllValid(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		...
		err := gpkjSubmissionTask.Initialize(
			...
			"GpkjSubmissionTask",
			"tak-id",
			gpkjSubmissionTask.Start,
			gpkjSubmissionTask.End,
			false,
			nil,
			nil,
		)
	...
	for idx, acct := range accounts {
		...
		assert.Nil(t, err)
		p, err := fixture.Contracts.EthereumContracts().
			Ethdkg().
			GetParticipantInternalState(callOpts, acct.Address)
		assert.Nil(t, err)
		...
		if (p.Gpkj[0].Cmp(stateGPKJ[0]) != 0) || (p.Gpkj[1].Cmp(stateGPKJ[1]) != 0) ||
			(p.Gpkj[2].Cmp(stateGPKJ[2]) != 0) ||
			(p.Gpkj[3].Cmp(stateGPKJ[3]) != 0) {
...
func TestGPKjSubmission_Group_2_Bad4(t *testing.T) {
	...
	dkgState.Participants[dkgState.Account.Address].GPKj = [4]*big.Int{
		big.NewInt(0),
		big.NewInt(0),
		big.NewInt(0),
		big.NewInt(0),
