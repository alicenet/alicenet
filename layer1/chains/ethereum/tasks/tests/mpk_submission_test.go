...
func TestMPKSubmission_Group_1_GoodAllValid(t *testing.T) {
	...
	for idx := 0; idx < numValidators; idx++ {
		...
		err := mpkSubmissionTask.Initialize(
			...
			"MpkSubmissionTasks",
			"tak-id",
			mpkSubmissionTask.Start,
			mpkSubmissionTask.End,
			false,
			nil,
			nil,
		)
	...
	for idx, acct := range accounts {
		...
		assert.Nil(t, err)
		isMPKSet, err := fixture.Contracts.EthereumContracts().
			Ethdkg().
			IsMasterPublicKeySet(callOpts)
		assert.Nil(t, err)
