...
func TestRegisterTask_Group_1_Task(t *testing.T) {
	...
	assert.Nil(t, err)
	err = registrationTask.Initialize(ctx, nil, dkgDb, fixture.Logger, eth, "RegistrationTask", "111", nil)
	assert.Nil(t, err)
...
func TestRegisterTask_Group_1_Good2(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		...
		err = tasksVec[idx].Initialize(ctx, nil, dkgDb, fixture.Logger, eth, "RegistrationTask", "111", nil)
		assert.Nil(t, err)
