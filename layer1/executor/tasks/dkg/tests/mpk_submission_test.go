...
func TestMPKSubmission_Group_1_Bad1(t *testing.T) {
	...
	err = task.Initialize(ctx, nil, suite.DKGStatesDbs[0], fixture.Logger, suite.Eth, fixture.Contracts, "MPKSubmissionTask", "task-id", task.Start, task.End, false, nil, nil)
	assert.Nil(t, err)
...
func TestMPKSubmission_Group_2_LeaderElection(t *testing.T) {
	...
	for idx := 0; idx < numValidators; idx++ {
		...
		err = task.Initialize(ctx, nil, suite.DKGStatesDbs[idx], fixture.Logger, suite.Eth, fixture.Contracts, "MPKSubmissionTask", "task-id", task.Start, task.End, false, nil, nil)
		assert.Nil(t, err)
