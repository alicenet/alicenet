...
func TestDisputeShareDistributionTask_Group_1_OneValidatorSubmittingInvalidCredentials(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		for j := 0; j < n; j++ {
			...
			err := task.Initialize(suite.DKGStatesDbs[idx], fixture.Logger, suite.Eth, fixture.Contracts, "DisputeShareDistributionTask", "task-id", task.Start, task.End, false, nil, nil)
			assert.Nil(t, err)
