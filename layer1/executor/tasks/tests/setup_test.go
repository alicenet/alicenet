...
func StartFromRegistrationOpenPhase(t *testing.T, fixture *tests.ClientFixture, unregisteredValidators int, phaseLength uint16) *EthDkgTestSuite {
	...
	for idx := 0; idx < n; idx++ {
		...
		err = regTasks[idx].Initialize(dkgDb, nil, logger, eth, fixture.Contracts, "RegistrationTask", fmt.Sprintf("%v", idx), regTasks[idx].Start, regTasks[idx].End, false, nil, nil)
		assert.Nil(t, err)
...
func StartFromShareDistributionPhase(t *testing.T, fixture *tests.ClientFixture, undistributedSharesIdx []int, badSharesIdx []int, phaseLength uint16) *EthDkgTestSuite {
	...
	for idx := 0; idx < n; idx++ {
		...
		err = shareDistTask.Initialize(suite.DKGStatesDbs[idx], nil, logger, suite.Eth, fixture.Contracts, "ShareDistributionTask", fmt.Sprintf("%v", idx), shareDistTask.Start, shareDistTask.End, false, nil, nil)
		assert.Nil(t, err)
...
func StartFromKeyShareSubmissionPhase(t *testing.T, fixture *tests.ClientFixture, undistributedShares int, phaseLength uint16) *EthDkgTestSuite {
	...
	for idx := 0; idx < n; idx++ {
		...
		err := keyshareSubmissionTask.Initialize(suite.DKGStatesDbs[idx], nil, logger, suite.Eth, fixture.Contracts, "KeyShareSubmissionTask", fmt.Sprintf("%v", idx), keyshareSubmissionTask.Start, keyshareSubmissionTask.End, false, nil, nil)
		assert.Nil(t, err)
...
func StartFromMPKSubmissionPhase(t *testing.T, fixture *tests.ClientFixture, phaseLength uint16) *EthDkgTestSuite {
	...
	for idx := 0; idx < numValidators; idx++ {
		...
		err := task.Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "MPKSubmissionTask", fmt.Sprintf("%v", idx), task.Start, task.End, false, nil, nil)
		assert.Nil(t, err)
...
func StartFromGPKjPhase(t *testing.T, fixture *tests.ClientFixture, undistributedGPKjIdx []int, badGPKjIdx []int, phaseLength uint16) *EthDkgTestSuite {
	...
	for idx := 0; idx < n; idx++ {
		...
		err := gpkjSubTask.Initialize(suite.DKGStatesDbs[idx], nil, logger, suite.Eth, fixture.Contracts, "GPKjSubmissionTask", fmt.Sprintf("%v", idx), gpkjSubTask.Start, gpkjSubTask.End, false, nil, nil)
		assert.Nil(t, err)
...
func CompleteEthDkgCeremony(t *testing.T, numValidators int) (*tests.ClientFixture, *EthDkgTestSuite) {
	...
	for idx := 0; idx < numValidators; idx++ {
		for j := 0; j < numValidators; j++ {
			...
			err := disputeGPKjTask.Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "disputeGPKjTask", "task-id", 0, 0, false, nil, nil)
			assert.Nil(t, err)
	...
	for idx := 0; idx < numValidators; idx++ {
		...
		err := completionTask.Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "CompletionTask", "task-id", 0, 0, false, nil, nil)
		assert.Nil(t, err)
