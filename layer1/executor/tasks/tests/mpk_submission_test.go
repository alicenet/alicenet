...
func TestMPKSubmission_Group_1_GoodAllValid(t *testing.T) {
	...
	for idx := 0; idx < numValidators; idx++ {
		...
		err := mpkSubmissionTask.Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "MpkSubmissionTasks", "tak-id", mpkSubmissionTask.Start, mpkSubmissionTask.End, false, nil, nil)
		assert.Nil(t, err)
...
func TestMPKSubmission_Group_1_Bad1(t *testing.T) {
	...
	err = task.Initialize(suite.DKGStatesDbs[0], nil, fixture.Logger, suite.Eth, fixture.Contracts, "MPKSubmissionTask", "task-id", task.Start, task.End, false, nil, nil)
	assert.Nil(t, err)
...
func TestMPKSubmission_Group_2_LeaderElection(t *testing.T) {
	...
	for idx := 0; idx < numValidators; idx++ {
		...
		err = task.Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "MPKSubmissionTask", "task-id", task.Start, task.End, false, nil, nil)
		assert.Nil(t, err)
