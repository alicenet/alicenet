...
func TestDisputeMissingKeySharesTask_FourUnsubmittedKeyShare_DoWork_Success(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		...
		err := disputeMissingKeyshareTask.Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "DisputeMissingKeyshareTask", "task-id", disputeMissingKeyshareTask.Start, disputeMissingKeyshareTask.End, false, nil, nil)
		assert.Nil(t, err)
