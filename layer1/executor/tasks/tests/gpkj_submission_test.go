...
func TestGPKjSubmission_Group_1_GoodAllValid(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		...
		err := gpkjSubmissionTask.Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "GpkjSubmissionTask", "tak-id", gpkjSubmissionTask.Start, gpkjSubmissionTask.End, false, nil, nil)
		assert.Nil(t, err)
...
func TestGPKjSubmission_Group_1_Bad1(t *testing.T) {
	...
	err = task.Initialize(suite.DKGStatesDbs[0], nil, fixture.Logger, suite.Eth, fixture.Contracts, "GpkjSubmissionTask", "task-id", task.Start, task.End, false, nil, nil)
	assert.Nil(t, err)
