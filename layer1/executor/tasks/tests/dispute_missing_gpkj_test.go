...
func TestDisputeMissingGPKjTask_Group_1_FourUnsubmittedGPKj_DoWork_Success(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		...
		err := disputeMissingGPKjTask.Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "DisputeMissingGPKjTask", "task-id", disputeMissingGPKjTask.Start, disputeMissingGPKjTask.End, false, nil, nil)
		assert.Nil(t, err)
