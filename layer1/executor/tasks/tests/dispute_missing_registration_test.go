...
func TestDisputeMissingRegistrationTask_Group_1_DoTaskSuccessOneParticipantAccused(t *testing.T) {
	...
	for idx := 0; idx < n; idx++ {
		err := suite.DispMissingRegTasks[idx].Initialize(suite.DKGStatesDbs[idx], nil, fixture.Logger, suite.Eth, fixture.Contracts, "DisputeMissingRegistrationTask", "task-id", suite.DispMissingRegTasks[idx].Start, suite.DispMissingRegTasks[idx].End, false, nil, nil)
		assert.Nil(t, err)
