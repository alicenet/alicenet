...
import { migrateSnapshotsAndValidators, stakeValidators } from "../../scripts/lib/alicenetTasks";
import {
...
describe("State Migration: Migrate state", () => {
  ...
  it("Should migrate using special migration contract", async function () {
    ...
    let txResponse = await stakeValidators(
      ...
      fixture.validatorPool.address, hre)
  ...
  const ethHeight = 566;
    await mineBlocks(2n);
  let txResponsePromise = await migrateSnapshotsAndValidators(
      fixture.factory.address,
      fixture.publicStaking.address,
      fixture.snapshots.address,
      fixture.ethdkg.address,
      fixture.validatorPool.address, tokenIds, valAccounts, valShares, ethHeight, mpk, bClaims, groupSignatures, hre)
