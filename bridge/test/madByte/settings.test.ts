...
describe("Testing MadByte Settings", async () => {
  ...
  it("Should fail to set split not being an admin", async () => {
    await expect(
      fixture.madByte.connect(user).setSplits(300, 300, 300, 100)
    ).to.be.revertedWith("Must be admin");
  });

  it("Should fail to set splits greater than one unit", async () => {
    await expect(
      fixture.madByte.connect(admin).setSplits(333, 333, 333, 2)
    ).to.be.revertedWith(
  ...
  it("Should fail to set all splits to 0", async () => {
    await expect(
      fixture.madByte.connect(admin).setSplits(0, 0, 0, 0)
    ).to.be.revertedWith(
      "MadByte: All the split values must sum to _MAD_UNIT_ONE!"
    );
  });
