...
contract crypto {
    ...
    // The negated form is required because bn128_pairing check in Solidity requires this.
    //
    ...
    // will be required as determined by indices.
    ...
    // bn128_add performs elliptic curve addition on the bn256 curve
    // of Ethereum.
    //
    // Copied from ETHDKG Solidity file and slightly modified
    ...
    // bn128_multiply performs elliptic curve scalar multiplication
    // on the bn256 curve of Ethereum.
    //
    // Copied from ETHDKG contract and slightly modified
    ...
    private view returns (uint256[2] memory result) {
        ...
        assembly {
            ...
            //              (256 bit for x, 256 bit for y, 256 bit for scalar)
            // 64       size of call return value,
            //              i.e. 64 bytes / 512 bit for a BN256 curve point
    ...
    // bn128_check_pairing performs a pairing check on the bn256 curve
    // of Ethereum; see Verify.
    //
    // Copied from ETHDKG contract and slightly modified
    ...
    private view returns (bool) {
        ...
        bool success;
        assembly {
            // 0x08     id of precompiled bn256Pairing contract
            //              (checking the elliptic curve pairings)
    ...
    // bn128_is_on_curve confirms that [x, y] is a valid elliptic curve point
    // on the elliptic curve for the group G1 of the bn256 curve
    // of Ethereum; see Verify.
    //
    // Copied from ETHDKG contract and slightly modified
    //
    // Does this work if point is Infinity?
    function bn128_is_on_curve(uint256[2] memory point)
    private pure returns(bool)
    {
        // check if the provided point is on the bn128 curve (y**2 = x**3 + curveB)
        return
