// Package cloudflare implements a particular bilinear group at the 128-bit security
// level.
//
...
// where Order is the order to the underlying groups.
...
func (e *G2) Unmarshal(m []byte) ([]byte, error) {
	...
	if e.p == nil {
		...
		e.p.x.x, e.p.x.y, e.p.y.x, e.p.y.y = gfP{0}, gfP{0}, gfP{0}, gfP{0}
	}
	var err error
...
// source groups to F_p^12. Miller(g1, g2).Finalize() is equivalent to
// Pair(g1, g2).
...
func (e *GT) Unmarshal(m []byte) ([]byte, error) {
	...
	if e.p == nil {
		...
		e.p.x.x.x, e.p.x.x.y, e.p.x.y.x, e.p.x.y.y, e.p.x.z.x, e.p.x.z.y = gfP{0}, gfP{0}, gfP{0}, gfP{0}, gfP{0}, gfP{0}
		e.p.y.x.x, e.p.y.x.y, e.p.y.y.x, e.p.y.y.y, e.p.y.z.x, e.p.y.z.y = gfP{0}, gfP{0}, gfP{0}, gfP{0}, gfP{0}, gfP{0}
	}
