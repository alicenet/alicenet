...
func SetupEventMap(em *objects.EventMap, cdb *db.Database, adminHandler monInterfaces.IAdminHandler, depositHandler monInterfaces.IDepositHandler, taskRequestChan chan<- interfaces.ITask, taskKillChan chan<- string) error {
	...
	processValidatorMinorSlashedFunc := func(eth ethereum.Network, logger *logrus.Entry, state *objects.MonitorState, log types.Log) error {
	...
	processValidatorMajorSlashedFunc := func(eth ethereum.Network, logger *logrus.Entry, state *objects.MonitorState, log types.Log) error {
