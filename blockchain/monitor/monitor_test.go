...
func TestMonitorPersist(t *testing.T) {
	...
	//
	newMon, err := NewMonitor(database, database, mocks.NewMockIAdminHandler(), &mockDepositHandler{}, eth, 1*time.Second, time.Minute, 1, make(chan interfaces.ITask, 10), make(chan string, 10))
	assert.Nil(t, err)
...
//	mockTsk := &mockTask{
//		DkgTask: objects2.NewTask(nil, "name", 1, 40),
//	}
//	// Schedule some tasks
