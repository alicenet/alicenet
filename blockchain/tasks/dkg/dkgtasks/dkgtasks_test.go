...
//
//	err := monitor.SetupEventMap(events, nil, adminHandler, nil, make(chan interfaces.ITask))
//	assert.NoError(t, err)
//
//	var done bool
//
//	for !done {
//		err := monitor.MonitorTick(ctx, cancel, wg, eth, monitorState, logger, events, adminHandler, 10, nil, make(chan uint64))
//		assert.Nil(t, err)
//
...
//	tr.RegisterInstanceType(&PlaceHolder{})
...
//	tr.RegisterInstanceType(&CompletionTask{})
//}
...
//	_, _, err = tasks.SetETHDKGPhaseLength(20, eth, txnOpts, ctx)
//	assert.Nil(t, err)
//
//	_, _, err = tasks.InitializeETHDKG(eth, txnOpts, ctx)
//	assert.Nil(t, err)
//
